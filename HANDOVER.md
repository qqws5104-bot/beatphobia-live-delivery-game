...
```
원본 PNG (저장소 밖, 전반+후반 2벌)  ──build_images.py──▶  /tmp/compressed/*.jpg  ──build_client.py──▶  public/index.html
   장당 ~835KB                                          장당 ~70KB              base64 인라인 (CELLS_1+CELLS_2)
   1920x1081                                             1280x720 q78
```

- **원본 PNG는 이 저장소에 없다.** 전반은 `/home/claude/project/quiz_board/ref/`, 후반은
  `/home/claude/project/quiz_board/ref_2/` (2026-08-28부터 분리 — 아래 콜아웃 참고).
  이미지를 바꿀 일이 없으면 신경 쓸 필요 없다 — 배포에 필요한 것은 이미 `public/index.html`에 들어 있다.
- 파일명은 `build_client.py`가 **정렬 순서만** 보고 각 하프의 CELLS[0..20]에 순서대로 배정하므로,
  이름 자체보다 **정렬했을 때의 순서**가 유일하게 중요하다. 지금은 `jeonban_01.png`~`jeonban_21.png`
  (전반), `huban_01.png`~`huban_21.png`(후반)처럼 두 자리 0-패딩을 줘서 문자열 정렬이 곧 숫자 순서가
  되도록 맞춰뒀다 — 패딩 없이 `_1.png`~`_21.png`라고만 하면 `_10.png`가 `_2.png`보다 앞에 와버려
  순서가 깨진다. 새 이미지 세트를 넣을 때마다 이 점을 반드시 지킬 것.

> **✅ 21칸 이미지 세트 도착 완료 (2026-08-27, "전반" 세트).** 사용자가 예고했던 대로("게임 우봉고
> 이미지는 내가 따로 더 줄테니, 전반과 후반을 각각 만들도록 해") 21장짜리 이미지 세트가 도착해서
> `quiz_board/ref/`의 옛 20장(+placeholder 재사용 흠결)을 완전히 교체했다. 파일명은
> `jeonban_01.png`~`jeonban_21.png`(0-패딩, 정렬 순서 = 사용자가 준 원래 순서 1~21 그대로). 순서는
> **배송 색상 스와치 개수**로 카테고리를 역산해서 확인함 — 1~5번(스와치 2개)=일반택배, 6~10번(3개)=
> 깨지기 쉬운 택배, 11~15번(4개)=귀중품, 16~21번(3개, 6장)=확정 층수 택배. TYPES 순서/count(5/5/5/6)와
> 정확히 일치해서, `build_client.py`가 하던 "마지막 이미지 재사용" 플레이스홀더 경고가 완전히
> 사라졌다(`len(IMAGE_FILES) == TOTAL_CELLS == 21`). 옛 20장은 `quiz_board/ref_backup_20260827/`에
> 백업해뒀다(혹시 몰라 보관, 딱히 쓸 일은 없을 것).
>
> **✅ 후반용 21장 세트 도착 및 반영 완료 (2026-08-28).** 예고했던 후반 세트가 도착해서 전반/후반
> 이미지를 완전히 분리했다: 원본 PNG는 `REF_DIR_1`(전반, `quiz_board/ref/`, `jeonban_01~21.png`)과
> `REF_DIR_2`(후반, `quiz_board/ref_2/`, `huban_01~21.png`, 0-패딩)로 나뉘고, `build_images.py`가
> 두 디렉터리를 각각 압축하며(파일명 접두사가 달라 `COMPRESSED_DIR` 하나를 같이 써도 안전),
> `build_client.py`는 `CELLS_1`/`CELLS_2` 두 벌을 만들어 둘 다 클라이언트에 심는다. 순서는 전반과
> 동일한 방식(배송 색상 스와치 개수)으로 확인함 — 1~5=일반택배(2개), 6~10=깨지기 쉬운 택배(3개),
> 11~15=귀중품(4개), 16~21=확정 층수 택배(3개, 6장). 클라이언트의 `cellMeta(id, half)`가 `state.half`
> 값(서버가 이미 브로드캐스트하던 필드)을 보고 두 세트 중 하나에서 이미지를 골라 퍼즐 오버레이에
> 띄운다 — **보드 구조(칸 id/종류/번호, `game-data.js`의 `CELLS`)는 서버 쪽에서 두 하프가 그대로
> 공유하며 바뀐 게 없다**, 오직 클라이언트가 보여주는 퍼즐 이미지만 하프별로 다르다. 빌드 후
> `CELLS_1`/`CELLS_2`에서 같은 id(예: `normal-1`)의 이미지를 추출해 실제로 다른 그림인지 육안 확인함
> (21칸 전부 서로 다른 src임을 프로그램으로도 확인). 옛 20장(2026-08-27 이전)은 여전히
> `quiz_board/ref_backup_20260827/`에 백업돼 있다.
...
```bash
# 1. 원본 PNG를 ref(전반)/ref_2(후반) 디렉터리에 덮어쓴다 (파일명 유지! 바꾸는 하프만 건드리면 됨)
# 2. 압축본 재생성  ← 빠뜨리기 쉬움 (build_images.py가 REF_DIR_1/REF_DIR_2 둘 다 압축함)
python3 build_images.py
# 3. 클라이언트 재빌드 (CELLS_1/CELLS_2 둘 다 재생성됨)
python3 build_client.py
# 4. 검증: 원본 21장이 전부 빌드에 들어갔는지 확인 (5.4절)
```
...
- **엘리베이터 층수 목록 가독성 개선** (`.shaft` / `.floor-stop`, 사용자 요청: "층수 잘 안보여서 잘
  보이게 바꿔줘. 엘레베이터 있는 위치가 눈에 띄도록") — 리스킨 직후엔 크림 배경(`#efdcb2`) 위에 흐린
  갈색(`var(--muted)`, `#8a7256`) 글자라 대비가 부족했고, 현재 층 강조도 14% 불투명도 골드 틴트뿐이라
  잘 안 띄었다. 그래서 (1) 나머지 층 글자를 `rgba(43,29,18,0.62)`(진한 잉크색), `font-weight:700`,
  `font-size:1.02rem`으로 진하게 키우고, (2) 현재 층 행은 **`var(--gold)` 단색 배경 + `var(--gold-ink)`
  크림 글자**(기존 primary 버튼과 같은 언어)로 바꿔 `font-weight:800`, `font-size:1.12rem`,
  `box-shadow:0 6px 16px rgba(226,105,26,0.45)`, `transform:scale(1.03)`까지 얹어 확실히 튀게 했다.
  엘리베이터 위치 표시 점(`.floor-stop .car`)도 10px 정사각형 → 11px 원형으로 바꾸고, 현재 층 점은
  `--gold-ink`로 채운 뒤 `box-shadow:0 0 0 4px rgba(255,248,239,0.35)`(옅은 흰 링)로 골드-온-골드라
  묻히던 문제를 없앴다.
...
그래서 실제 브랜드와 무관한 5개 신규 브랜드를 새로 지었다 (아래 표는 **2026-08-28 리스킨 이후
현재 값** — 최초 5종은 번개특급/왕눈이택배/두더지퀵/로켓아저씨/구름배송이었다):

| key | 이름 | 색 | 아이콘 모티브 |
|---|---|---|---|
| `cookbang` | 쿡방 | `#D9552E` | 프라이팬 + 계란 |
| `cheonil` | 천일배송 | `#7A2333` | 다이아몬드 직인 + 바코드선 |
| `hanjim` | 한짐택배 | `#1E4C86` | 방패 + 상자 |
| `mz` | MZ로지스틱스 | `#2F9E6E` | 지그재그 화살표 |
| `tongan` | 우체통안 | `#C23B3B` | 우편함 |

> **2026-08-28 리스킨 메모**: 사용자가 새 로고 스티커 이미지 5종을 주면서 이걸로 바꿔달라고
> 요청. 그중 "한진택배"는 실제 존재하는 택배회사명과 정확히 같아서, 위 3.6절 첫머리에 적힌
> "실제 브랜드는 상표권 문제로 안 쓴다"는 원칙과 정면으로 충돌한다고 판단해 AskUserQuestion으로
> 재확인. 사용자가 **말장난("한짐 가득 싣습니다")은 유지하되 이름만 한 글자 바꾼 "한짐택배"**로
