...
- 파일명은 `build_client.py`가 **정렬 순서만** 보고 CELLS[0..20]에 순서대로 배정하므로, 이름 자체보다
  **정렬했을 때의 순서**가 유일하게 중요하다. 지금은 `jeonban_01.png`~`jeonban_21.png`처럼 두 자리
  0-패딩을 줘서 문자열 정렬이 곧 숫자 순서가 되도록 맞춰뒀다 — 패딩 없이 `jeonban_1.png`~
  `jeonban_21.png`라고만 하면 `jeonban_10.png`가 `jeonban_2.png`보다 앞에 와버려 순서가 깨진다.
  새 이미지 세트를 넣을 때마다 이 점을 반드시 지킬 것.
...
>
> **⚠️ 아직 남은 일: 후반용 세트는 별도로 안 왔다.** 지금 구조는 전반/후반이 여전히 **같은 이미지
> 풀 하나**(`REF_DIR`)를 공유한다 — 즉 이번에 넣은 21장이 전반뿐 아니라 후반에도 그대로 보인다.
> 사용자가 예고한 "후반용" 21장이 따로 오면, 그때 가서:
> 1. `REF_DIR_1`(전반)/`REF_DIR_2`(후반) 같은 분리 구조로 바꾸고 `build_client.py`/`build_images.py`를
>    half별로 두 번 돌리거나, half별 CELLS를 따로 만드는 식으로 확장해야 한다. 후반 세트의 실제
>    전달 형태(폴더 구조, 21장 맞는지, 파일명 패턴)를 모르므로 미리 만들어두지 않았다 — 올 때 맞춰서.
> 2. 그 전까지는 전반/후반 둘 다 이번 21장(전반용)을 보여주는 게 현재 상태이며, 버그가 아니라
>    "후반 세트 아직 미도착"으로 인한 임시 상태임을 인지할 것.
