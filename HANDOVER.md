...
- **`.board-label`(보드 왼쪽 종류 카드) 재설계** — 예전엔 종류 색을 카드 전체 배경으로 칠했는데,
  이제 참고 포스터처럼 **크림 카드 + 종류 색 테두리 + 위에 플랫 라인 아이콘 + 이름, 아래에 오렌지
  헤더가 달린 "구분/요금" 미니 표**로 바뀌었다(`renderBoard`, `catIdx`로 `CAT_ICONS[catIdx]` 참조).
  아이콘은 실제 `<table>`이 아니라 grid로 짠 것 — 이 칸이 110~150px 폭에 4행 보드 높이 안에 맞아야
  해서, 표 태그의 기본 여백을 이길 필요가 있었다(시각적으로는 표와 동일하게 읽힘). `CAT_ICONS`는
  TYPES 순서와 동일한 4개의 손으로 그린 flat line SVG(트럭/깨진 유리잔/다이아몬드/체크마크 건물) —
  `box_art/`의 입체 박스 일러스트와는 별개다(그 일러스트는 라벨 카드 안 아이콘 크기로 쓰기엔 스타일이
  안 맞음). 박스 칸 자체(`.cell`, `.cell-num`, `.invoice-label` — 크림+네이비 라벨판)는 이 리스킨과
  무관하게 그대로 뒀다: 그건 사용자가 준 박스 일러스트 자체의 라벨판 모티브에 맞춘 디자인이라, 페이지
  전체 테마가 바뀌어도 독립적으로 유지되는 게 맞다는 판단.
