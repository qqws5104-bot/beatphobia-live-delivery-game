...
| 순서 | 종류 | 칸 수 | 배송 성공 | 실패(미배송/도난) | 배경색 | 칸 표기 |
|---|---|---|---|---|---|---|
| 1 | 일반택배 | 5 | **+2,500원** | **-1,000원** | 탠(`#C9A576`) | a~e |
| 2 | 깨지기 쉬운 택배 | 5 | **+5,000원** | **-2,500원** | 연두(`#C7E29A`) | a~e |
| 3 | 귀중품 | 5 | **+10,000원** | **-5,000원** | 골드(`#F0B84A`) | a~e |
| 4 | 확정 층수 택배 | **6** | **+3,000원** | **-1,500원** | 스카이(`#6DBBFD`) | **B1/1F/2F/3F/4F/5F 그대로** |
...
칸 번호 대신 `a`~`e`로 표기한다 (2026-08-27, `renderBoard`의 `CELL_LETTERS`).
...
**칸 디자인 (2026-08-27 재작업)**: 예전의 "골판지 상자 + 십자 접힘선 + 세로 테이프" 느낌 대신,
둥근 모서리의 입체적("clay render") 상자 느낌으로 바꿨다 — 사용자가 참고 이미지(클립아트 사이트의
워터마크 있는 3D 박스 렌더)를 제시했으나, **그 이미지 파일 자체는 저작권이 있는 스톡 이미지라
그대로 가져다 쓰지 않았고**, 대신 그 스타일(둥근 모서리, 좌상단 광택, 대각선 띠, 우하단 작은
태그+줄 아이콘)을 순수 CSS로 재현했다 (`build_client.py`의 `.cell`/`.cell::before`/`.cell::after`/
