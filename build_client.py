...
<style>
  ...
  * { box-sizing: border-box; }
  ...
  /* Elevator position is drawn as a filling gauge rather than a list of rows. The two players share
     one car and each click moves it a notch, so what matters visually is the LEVEL rising and
     falling as they fight over it -- a highlight jumping between rows doesn't read as movement.
     Six equal notches means the fill always lands exactly on a floor boundary, so "which floor" is
     never ambiguous (delivery addresses are floor-bound, e.g. 401호). */
  .shaft { background:var(--bg-deep); border-radius:14px; border:1px solid var(--panel-line); padding:0.9rem 0.8rem; }
  .gauge { position:relative; height:clamp(250px,46vh,360px); border-radius:10px; overflow:hidden;
    background:linear-gradient(180deg,rgba(0,0,0,0.38),rgba(0,0,0,0.16)); border:1px solid var(--panel-line); }
  .gauge-fill { position:absolute; left:0; right:0; bottom:0;
    background:linear-gradient(180deg,#ffd479 0%,var(--gold) 20%,#cf8f26 100%);
    box-shadow:0 0 20px rgba(240,184,74,0.4);
    transition:height 200ms cubic-bezier(0.22,1,0.36,1); }
  /* bright lip along the top of the fill -- gives the level a visible "surface", which is what
     makes a one-notch change read as the gauge rising instead of a row simply changing color */
  .gauge-fill::after { content:""; position:absolute; left:0; right:0; top:0; height:3px;
    background:#fff4d6; box-shadow:0 0 10px rgba(255,244,214,0.85); }
  .gauge-rows { position:absolute; inset:0; display:flex; flex-direction:column-reverse; }
  .floor-stop { flex:1; min-height:0; display:flex; align-items:center; gap:0.45rem; padding:0 0.65rem;
  ...
  .floor-stop:first-child { border-bottom:none; } /* column-reverse: first child sits at the very bottom */
  /* submerged floors sit on the gold fill, so they need dark ink instead of the light default */
  .floor-stop.filled { color:#46300a; }
  .floor-stop.current { color:#2a1c02; font-weight:700; font-size:1.1rem; }
  .floor-stop .car { width:9px; height:9px; border-radius:2px; flex:none; background:transparent;
    transition:background-color 200ms ease, box-shadow 200ms ease; }
  .floor-stop.current .car { background:#2a1c02; box-shadow:0 0 0 3px rgba(255,255,255,0.4); }
  @media (max-width:820px) { .gauge { height:210px; } }
...
(function () {
  ...
  // clicks (see game-room.js's vote()), so the gauge is a direct rendering of it with no
  // client-side simulation whatsoever. Fill height is (floorIdx+1)/6, i.e. always exactly on a
  // notch boundary.
  var prevShaftFloorIdx = null; // level the gauge was last drawn at, so a re-render can animate FROM it
  function shaftFillPct(i) { return ((i + 1) / FLOORS.length) * 100; }

  function renderShaft(floorIdx) {
    // render() tears down and rebuilds the entire DOM on every broadcast, so a fill element created
    // at its final height would simply appear there with no motion at all. Draw it at the PREVIOUS
    // level and let settleShaft() move it to the real one on the next frame -- that gives the CSS
    // transition something to animate from, which is what makes a click read as the level rising or
    // falling rather than a row just relighting.
    var from = prevShaftFloorIdx === null ? floorIdx : prevShaftFloorIdx;
    prevShaftFloorIdx = floorIdx;
    var html = '<div class="shaft"><div class="gauge">'
      + '<div class="gauge-fill" style="height:' + shaftFillPct(from) + '%" data-target="' + shaftFillPct(floorIdx) + '"></div>'
      + '<div class="gauge-rows">';
    // DOM order stays FLOORS order (B1 first) -- the bottom-to-top stacking comes from the rows'
    // column-reverse, so rows[i] is still floor i no matter where it sits on screen. Tests rely on
    // that mapping.
    FLOORS.forEach(function (f, i) {
      var cls = "floor-stop";
      if (i <= floorIdx) cls += " filled"; // submerged: sits on the gold fill, needs dark ink
      if (i === floorIdx) cls += " current";
      html += '<div class="' + cls + '"><span class="car"></span>' + f + '</div>';
    });
    html += '</div></div></div>';
    return html;
  }

  // Moves the freshly-rendered gauge from the level it was drawn at to the real one, one frame
  // later, so the height change is a transition instead of an initial layout. Never blocks input:
  // a click landing mid-animation just triggers another render, which starts from wherever the
  // level logically was and animates on to the new one.
  function settleShaft() {
    var fill = document.querySelector(".gauge-fill");
    if (!fill) return;
    var target = fill.getAttribute("data-target");
    if (target === null) return;
    // Two nested frames on purpose: with only one, the browser folds the "from" height and the
    // "to" height into the same initial layout and skips the transition entirely.
    requestAnimationFrame(function () {
      requestAnimationFrame(function () { fill.style.height = target + "%"; });
    });
  }
  ...
  function renderElevator(st, seat) {
    ...
    // live in the gauge, is the only movement feedback either player gets during voting.
  ...
  function render() {
    ...
    settleShaft(); // no-op unless the elevator gauge is on screen
  }
