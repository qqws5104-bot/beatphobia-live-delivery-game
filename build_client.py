...
<style>
  ...
  * { box-sizing: border-box; }
  ...
  /* the actual floor only changes once the 5s vote window resolves (aggregate tally, unchanged),
     but every single up/down click -- mine or the opponent's -- gives the current-floor row an
     immediate little tug in that direction, so voting feels responsive rather than silent until
     the round ends. Purely cosmetic: no numbers are shown, just a nudge. */
  @keyframes floorNudgeUp { 0% { transform:translateY(0); } 35% { transform:translateY(-9px); } 100% { transform:translateY(0); } }
  @keyframes floorNudgeDown { 0% { transform:translateY(0); } 35% { transform:translateY(9px); } 100% { transform:translateY(0); } }
  .floor-stop.nudge-up { animation:floorNudgeUp 0.4s ease; }
  .floor-stop.nudge-down { animation:floorNudgeDown 0.4s ease; }
...
(function () {
  ...
  // ---------- elevator car nudge: purely cosmetic per-click feedback (see the .floor-stop.nudge-*
  // CSS above) -- diffs the previous vote tally against the new one to see whether an up or down
  // click just landed (from EITHER player -- their exact counts stay hidden, only the fact that
  // something just moved is used), then gives the current-floor row a quick tug in that direction.
  // The actual floor still only changes once the round resolves from the full aggregate. ----------
  ...
  function bumpCar(dir) {
    var row = document.querySelector(".floor-stop.current");
    if (!row) return;
    row.classList.remove("nudge-up", "nudge-down");
    void row.offsetWidth; // force reflow so re-adding the same class restarts the animation
    row.classList.add(dir === "up" ? "nudge-up" : "nudge-down");
  }
